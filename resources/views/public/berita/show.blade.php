    ...
    <div class="container-xl relative z-10">
        <div class="max-w-4xl mx-auto">
            <nav class="flex items-center gap-2 text-sm text-white/50">
                ...
                <a href="{{ route('berita.index') }}" class="hover:text-white/80 transition-colors">Berita</a>
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                <span class="text-white/80 line-clamp-1">{{ $berita->judul }}</span>
            </nav>
        </div>
    </div>
</section>
...
<section class="section-padding bg-white">
    <div class="container-xl max-w-4xl">
        ...
        {{-- Share --}}
        <div class="flex flex-wrap items-center gap-3 py-5 border-t border-b border-neutral-100 mb-10">
            ...
            <a href="https://wa.me/?text={{ urlencode($berita->judul.' '.url()->current()) }}" target="_blank" rel="noopener"
                class="inline-flex items-center gap-1.5 text-xs font-medium text-neutral-600 hover:text-primary-600 bg-neutral-100 hover:bg-primary-50 px-3 py-1.5 rounded-lg transition-colors">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <article class="bg-neutral-50 rounded-xl overflow-hidden card-hover">
                    <div class="aspect-video bg-gradient-to-br from-primary-100 to-primary-200 flex items-center justify-center overflow-hidden">
                        @if($rel->thumbnail)
                        <img src="{{ Storage::url($rel->thumbnail) }}" alt="{{ $rel->judul }}" class="w-full h-full object-cover" loading="lazy">
                        @else
