...
@section('breadcrumb')
    <a href="{{ route('admin.galeri.index') }}" class="text-neutral-500 hover:text-primary-600 text-sm transition-colors">Galeri Foto</a>
...
<div class="max-w-2xl">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        <h1 class="text-xl font-bold text-neutral-900">Detail Foto</h1>
    ...
    <div class="bg-white rounded-2xl border border-neutral-200 overflow-hidden">
        @if($galeri->file)
        <img src="{{ Storage::url($galeri->file) }}" alt="{{ $galeri->judul }}" class="w-full max-h-96 object-cover bg-neutral-100">
        @endif
        <div class="px-6 py-5">
            <div class="flex items-start justify-between gap-4 mb-4">
                <h2 class="font-bold text-neutral-900 text-lg">{{ $galeri->judul }}</h2>
                @if($galeri->is_active)
            ...
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                <div>
                    <dt class="text-xs font-medium text-neutral-400 uppercase tracking-wide mb-1">Album</dt>
                    <dd class="text-sm text-neutral-800">{{ $galeri->album ?: '-' }}</dd>
                </div>
                <div>
                    <dt class="text-xs font-medium text-neutral-400 uppercase tracking-wide mb-1">Kategori</dt>
                    <dd class="text-sm text-neutral-800">{{ $galeri->kategori->nama ?? '-' }}</dd>
                </div>
                <div>
                    <dt class="text-xs font-medium text-neutral-400 uppercase tracking-wide mb-1">Urutan</dt>
                    <dd class="text-sm text-neutral-800">{{ $galeri->urutan ?? 0 }}</dd>
                </div>
                <div>
                    <dt class="text-xs font-medium text-neutral-400 uppercase tracking-wide mb-1">Diunggah</dt>
                    <dd class="text-sm text-neutral-800">{{ $galeri->created_at->format('d M Y, H:i') }}</dd>
                </div>
                @if($galeri->keterangan)
                <div class="sm:col-span-2">
                    <dt class="text-xs font-medium text-neutral-400 uppercase tracking-wide mb-1">Keterangan</dt>
                    <dd class="text-sm text-neutral-700 leading-relaxed">{{ $galeri->keterangan }}</dd>
                </div>
                @endif
            </dl>
        </div>
        <div class="px-6 py-4 border-t border-neutral-100 flex flex-wrap items-center gap-3">
            ...
            <form action="{{ route('admin.galeri.destroy', $galeri) }}" method="POST"
                data-confirm="Foto ini akan dihapus permanen dan tidak dapat dikembalikan." data-confirm-title="Hapus Foto?" data-confirm-ok="Ya, Hapus" data-confirm-variant="danger">
                @csrf @method('DELETE')
